...
```json
{
  "scripts": {
    ...
    "post-update-cmd": "test ! -f vendor/bin/rector || vendor/bin/rector process"
  }
}
```
...
Хук `post-update-cmd` автоматически применяет миграции после каждого `composer update` — код проекта сразу приводится к актуальному API обновлённых зависимостей, остаётся проверить дифф и прогнать тесты. Проверка `test ! -f vendor/bin/rector` нужна, чтобы хук молча пропускался при обновлении без dev-зависимостей (`composer update --no-dev`). Миграции идемпотентны: повторный запуск по уже обновлённому коду ничего не меняет.
...
- **Префикс со словом Migration и датой-временем** в формате `YYYYMMDD_HHMM` — рекомендуемый способ получить стабильный, воспроизводимый порядок применения:
